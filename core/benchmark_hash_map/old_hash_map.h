...
// struct HashMapElement1 {
// 	HashMapElement1 *next = nullptr;
// 	HashMapElement1 *prev = nullptr;
// 	KeyValue<TKey, TValue> data;
// 	HashMapElement1() {}
// 	HashMapElement1(const TKey &p_key, const TValue &p_value) :
// 			data(p_key, p_value) {}
// };
...
// 		typename Allocator = DefaultTypedAllocator<HashMapElement1<TKey, TValue>>>
// class HashMap1 {
// public:
...
// 	HashMapElement1<TKey, TValue> **elements = nullptr;
// 	uint32_t *hashes = nullptr;
// 	HashMapElement1<TKey, TValue> *head_element = nullptr;
// 	HashMapElement1<TKey, TValue> *tail_element = nullptr;
...
// 	void _insert_with_hash(uint32_t p_hash, HashMapElement1<TKey, TValue> *p_value) {
// 		const uint32_t capacity = hash_table_size_primes[capacity_index];
// 		const uint64_t capacity_inv = hash_table_size_primes_inv[capacity_index];
// 		uint32_t hash = p_hash;
// 		HashMapElement1<TKey, TValue> *value = p_value;
// 		uint32_t distance = 0;
// 		uint32_t pos = fastmod(hash, capacity_inv, capacity);
...
// 		HashMapElement1<TKey, TValue> **old_elements = elements;
// 		uint32_t *old_hashes = hashes;

// 		num_elements = 0;
// 		hashes = reinterpret_cast<uint32_t *>(Memory::alloc_static(sizeof(uint32_t) * capacity));
// 		elements = reinterpret_cast<HashMapElement1<TKey, TValue> **>(Memory::alloc_static(sizeof(HashMapElement1<TKey, TValue> *) * capacity));
...
// 	_FORCE_INLINE_ HashMapElement1<TKey, TValue> *_insert(const TKey &p_key, const TValue &p_value, bool p_front_insert = false) {
// 		uint32_t capacity = hash_table_size_primes[capacity_index];
...
// 			HashMapElement1<TKey, TValue> *elem = element_alloc.new_allocation(HashMapElement1<TKey, TValue>(p_key, p_value));
...
// 		uint32_t pos = 0;
// 		bool exists = _lookup_pos(p_key, pos);
// 		CRASH_COND_MSG(!exists, "HashMap1 key not found.");
...
// 		HashMapElement1<TKey, TValue> *element = elements[pos];
...
// 		// Update the HashMapElement1 with the new key and reinsert it.
...
// 		_FORCE_INLINE_ ConstIterator(const HashMapElement1<TKey, TValue> *p_E) { E = p_E; }
...
// 	private:
// 		const HashMapElement1<TKey, TValue> *E = nullptr;
// 	};
...
// 		_FORCE_INLINE_ Iterator(HashMapElement1<TKey, TValue> *p_E) { E = p_E; }
...
// 	private:
// 		HashMapElement1<TKey, TValue> *E = nullptr;
// 	};
...
// 	HashMap1(const HashMap1 &p_other) {
...
// 	void operator=(const HashMap1 &p_other) {
// 		if (this == &p_other) {
...
// 	HashMap1(uint32_t p_initial_capacity) {
// 		// Capacity can't be 0.
// 		capacity_index = 0;
...
private:
	...
	_FORCE_INLINE_ HashMapElement<TKey, TValue> *_insert_with_hash(const TKey &p_key, const TValue &p_value, uint32_t p_hash, bool p_front_insert = false) {
		...
		} else if (unlikely(num_elements + 1 > MAX_OCCUPANCY * capacity)) {
...
public:
	...
	void operator=(const HashMap1 &p_other) {
		if (this == &p_other || p_other.elements == nullptr) {
			return;
		}
		if (num_elements != 0) {
			clear();
...
private:
	...
	static _FORCE_INLINE_ uint32_t _get_resize_count(uint32_t p_capacity) {
		return p_capacity ^ (p_capacity + 1) >> 2; // = get_capacity() * 0.75 - 1;
	}
	...
	_FORCE_INLINE_ void _construct(uint32_t p_pos, uint32_t p_hash, HashMapElement<TKey, TValue> *p_value) {
		elements[p_pos] = p_value;
		hashes[p_pos] = p_hash;
		num_elements++;
		return;
	}
...
public:
	...
	TValue &get(const TKey &p_key) {
		uint32_t pos = 0;
		bool exists = _lookup_pos(p_key, pos);
		CRASH_COND_MSG(!exists, "HashMap2 key not found.");
		return elements[pos]->data.value;
	}
	...
	HashMap2(const HashMap2 &p_other) {
		capacity = p_other.capacity;
		if (p_other.num_elements == 0 || p_other.elements == nullptr) {
			return;
		}
	...
	void operator=(const HashMap2 &p_other) {
		if (this == &p_other || p_other.elements == nullptr) {
			return;
		}
		if (num_elements != 0) {
	...
	HashMap2(uint32_t p_initial_capacity) {
		capacity = MAX(4u, p_initial_capacity);
		capacity = next_power_of_2(capacity) - 1;
	}
	constexpr HashMap2() :
...
private:
	...
	static _FORCE_INLINE_ uint32_t _get_probe_length(const uint32_t p_pos, const uint32_t p_hash, const uint32_t p_capacity, const uint64_t p_capacity_inv) {
		...
		if (unlikely(p_pos < original_pos)) {
			return p_capacity - original_pos + p_pos;
		}
...
public:
	...
	TValue &get(const TKey &p_key) {
		uint32_t pos = 0;
		bool exists = _lookup_pos(p_key, pos);
		CRASH_COND_MSG(!exists, "HashMap4 key not found.");
		return elements[pos]->data.value;
	}
	...
	void reserve(uint32_t p_new_capacity) {
		...
		if (new_index == capacity_index && elements != nullptr) {
			return;
		}
	...
	HashMap4(const HashMap4 &p_other) {
		reserve(hash_table_size_primes[p_other.capacity_index]);
	...
	void operator=(const HashMap4 &p_other) {
		if (this == &p_other) {
			return; // Ignore self assignment.
		}
		if (num_elements != 0) {
	...
	HashMap4(uint32_t p_initial_capacity) {
		reserve(p_initial_capacity);
	}
	HashMap4() {
		_resize_and_rehash(MIN_CAPACITY_INDEX);
	}
